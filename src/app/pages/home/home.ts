  ...
  template: `
    <div class="min-h-screen">
      ...
      <div class="max-w-7xl mx-auto px-10 py-10 flex space-x-12 overflow-x-auto border-b border-rose-50 scrollbar-hide">
        <button class="text-xs font-black tracking-[0.3em] uppercase text-gray-900 border-b-2 border-gold pb-2">Todos los Productos</button>
        <button class="text-xs font-bold tracking-[0.3em] uppercase text-gray-300 hover:text-gray-900 transition-colors pb-2 border-b-2 border-transparent">Collares</button>
        <button class="text-xs font-bold tracking-[0.3em] uppercase text-gray-300 hover:text-gray-900 transition-colors pb-2 border-b-2 border-transparent">Pendientes</button>
        <button class="text-xs font-bold tracking-[0.3em] uppercase text-gray-300 hover:text-gray-900 transition-colors pb-2 border-b-2 border-transparent">Anillos</button>
      </div>
      ...
      <main class="max-w-7xl mx-auto px-10 py-24">
          ...
          <div *ngFor="let prod of products" class="group relative flex flex-col items-center text-center">
            <div [routerLink]="['/product', prod.slug]" class="relative w-full aspect-[3/4] bg-[#fcf9f8] rounded-[3rem] overflow-hidden mb-8 shadow-sm group-hover:shadow-2xl transition-all duration-700 cursor-pointer">
              ...
              <div class="absolute bottom-10 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                <button (click)="$event.stopPropagation(); addToCart(prod)" class="bg-white text-gray-900 px-8 py-3 rounded-full font-black text-[10px] tracking-[0.2em] uppercase shadow-xl hover:bg-gold hover:text-white transition-all whitespace-nowrap">Añadir Rápido</button>
              </div>
            </div>
        ...
        <div *ngIf="products.length === 0" class="py-40 text-center">
            <p class="text-sm font-serif italic text-gray-400 uppercase tracking-[0.4em]">La bóveda está vacía actualmente</p>
            <p class="mt-4 text-[10px] font-bold text-gray-300 tracking-widest uppercase">Suscríbete para recibir notificaciones del próximo lanzamiento</p>
        </div>
      </main>
      ...
      <footer class="bg-white px-10 py-32 border-t border-rose-50">
        <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-32">
          <div class="space-y-10">
            ...
            <p class="text-xl text-gray-400 font-light leading-relaxed max-w-sm">Curando elegancia artesanal desde 2024. Cada pieza cuenta una historia de maestría y amor.</p>
          </div>
          <div class="grid grid-cols-2 gap-10">
             <div class="space-y-6">
               <h4 class="text-[10px] font-black uppercase tracking-[0.3em] text-gray-900">Experiencia</h4>
               <ul class="space-y-4 text-sm text-gray-400 font-medium">
                 <li><a href="#" class="hover:text-gold transition-colors">Nuestra Historia</a></li>
                 <li><a href="#" class="hover:text-gold transition-colors">Cuidado Artesano</a></li>
                 <li><a href="#" class="hover:text-gold transition-colors">Diseño Personalizado</a></li>
               </ul>
             </div>
             <div class="space-y-6">
               <h4 class="text-[10px] font-black uppercase tracking-[0.3em] text-gray-900">Conexiones</h4>
               <ul class="space-y-4 text-sm text-gray-400 font-medium">
                 <li><a href="#" class="hover:text-gold transition-colors">Instagram</a></li>
                 <li><a href="#" class="hover:text-gold transition-colors">Diario</a></li>
               </ul>
             </div>
          </div>
        </div>
        <div class="max-w-7xl mx-auto pt-20 mt-20 border-t border-rose-50 flex justify-between items-center">
          <p class="text-[10px] font-bold text-gray-300 tracking-widest uppercase">© 2024 curado con amor</p>
